...
/// An item in a menu created by a [CustomSpinner].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class SpinnerItem<T> extends StatelessWidget {
  ...
  ///
  /// Eventually returned in a call to [CustomSpinner.onChanged].
  final T value;
...
/// An inherited widget that causes any descendant [CustomSpinner]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [CustomSpinner] widgets placed within material data tables, as
...
class DropdownButtonHideUnderline extends InheritedWidget {
  ...
  /// Returns whether the underline of [CustomSpinner] widgets should
  /// be hidden.
...
/// Typically, an enum is used. Each [SpinnerItem] in [items] must be
...
/// See also:
///
///  * [SpinnerItem], the class used to represent the [items].
...
class CustomSpinner<T> extends StatefulWidget {
  ...
  }) : assert(items == null || value == null || items.where((SpinnerItem<T> item) => item.value == value).length == 1),
        super(key: key);
...
class _DropdownButtonState<T> extends State<CustomSpinner<T>> with WidgetsBindingObserver {
  int _selectedIndex;
  ...
  @override
  void didUpdateWidget(CustomSpinner<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items.where((SpinnerItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
  ...
  Widget build(BuildContext context) {
    ...
    _enabled ? widget.hint : SpinnerItem<Widget>(child: widget.disabledHint ?? widget.hint);
...
/// A convenience widget that wraps a [CustomSpinner] in a [FormField].
class DropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [CustomSpinner] widget wrapped in an [InputDecorator] and
  /// [FormField].
  ///
  ...
  DropdownButtonFormField({
    Key key,
    T value,
    @required List<SpinnerItem<T>> items,
    this.onChanged,
